...
{
	double volume = (1.333333) * PI * pow(r, 3);
	return volume;
}
...
{
	double surfaceArea = 4 * PI * pow(r, 2);
	return surfaceArea;
}
...
{
	double surfaceArea = (2 * PI * r * h) + (2 * PI * pow(r, 2));
	return surfaceArea;
}
...
{
	double surfaceArea = PI * r * (r + sqrt(pow(h, 2) + pow(r, 2)));
	return surfaceArea;
}
...
and a cone using the standalone radius and height of which you will be prompted to input.\n" << endl;
	cout << "Let's get started. You will now be taken to the MAIN CONSOLE.\n" << endl;
	cout << "LOADING...............\n\n" << endl;
	MainII();
